...
library VehicleManufacturerStorage {
    ...
    function getName(address _storageAccount, bytes32 _name) public view returns(bytes32) {
        return EternalStorage(_storageAccount).getBytes32Value(
            keccak256(abi.encodePacked(_name, "name")));
    }
    ...
    function getOwner(address _storageAccount, bytes32 _name) public view returns(address) {
        return EternalStorage(_storageAccount).getAddressValue(
            keccak256(abi.encodePacked(_name, "owner")));
    } 
    ...
    function setOwner(address _storageAccount, bytes32 _name, address _owner) public {
        return EternalStorage(_storageAccount).setAddressValue(
            keccak256(abi.encodePacked(_name, "owner")), _owner);
    } 
